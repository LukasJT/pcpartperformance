...
if(require.main===module)(async()=>{
 ...
 const queue=listings.flatMap(([id,item,mpn])=>['CA','US'].map(region=>({id,item,mpn,region})));
 await Promise.all(Array.from({length:3},async()=>{while(queue.length){const p=queue.shift(),url=`https://www.newegg.${p.region==='CA'?'ca':'com'}/p/${p.item}`;try{
