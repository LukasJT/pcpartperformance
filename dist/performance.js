...
function renderLibrary(gpu,cpu){
 ...
 $('#game-library-grid').innerHTML=rows.map(g=>{const e=estimate(g,gpu,cpu),selected=g.id===gameSelect.value;return `<button class="game-tile${selected?' selected':''}" type="button" data-game-id="${esc(g.id)}" aria-pressed="${selected}"><span class="game-art">${image(g)}<span class="game-fps-chip">${Math.round(e.average)} <small>FPS</small></span></span><span class="game-tile-copy"><small>${g.year}</small><strong>${esc(g.name)}</strong><span>${esc(g.genre)}<i>${resolutionLabel($('#fps-resolution').value)} / ${$('#fps-quality').value}</i></span></span></button>`}).join('')||'<p class="console-empty">No games match those filters.</p>';
...
$('#game-reset').addEventListener('click',()=>{gameQuery='';gameGenre='all';showAll=false;$('#game-search').value='';$$('[data-game-genre]').forEach(b=>{const active=b.dataset.gameGenre==='all';b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active))});calculate()});
